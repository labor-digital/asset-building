...
const ComponentHelpers = require("./ComponentHelpers");
...
class ComponentRepository {
	...
	static getDefinitionFor(filename, modifiers) {
		...
		// Parse modifiers
		let parsedModifiers = ComponentHelpers.parseModifiers(modifiers);

		// Try to load from cache
		filename = FileHelpers.unifyFilename(filename);
		const cacheKey = path.dirname(filename) + "-" + parsedModifiers.get("raw");
		...
		const entryPoints = ComponentHelpers.resolveComponentEntryPointsForFilename(filename, parsedModifiers);
