	...
	static generateConfigFor(dir){
		...
		if (typeof laborConfig.plugins !== 'undefined' && Array.isArray(laborConfig.plugins)) {
			laborConfig.plugins.forEach(v => {
				let plugin = null;
				for(let basePath in [dir.buildingNodeModules, dir.nodeModules, dir.current]){
					try {
		...
		let builderVersion = typeof laborConfig.version === 'undefined' ? 1 : parseInt(laborConfig.version);
