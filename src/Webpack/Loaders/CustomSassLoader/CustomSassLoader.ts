...
const customSassLoader = function (this: any, source: string) {
    ...
    const errorHandler = function (e: any) {
        ...
        if (e.file) {
            if (e.file === 'stdin') {
                e.file = that.resourcePath;
            } else {
                e.file = path.normalize(e.file);
            }
                    ...
                    'custom-sass-loader-url-resolver($url: "", $filename: "")':
                        function customSassLoaderUrlResolver(url: any, filename: any) {
                            ...
                            if (fs.existsSync(url)) {
                                return new sass.types.String(pathRelativizer(url) + queryString);
                            }
                            ...
                            if (context.path.length > 0) {
                                ...
                                if (fs.existsSync(urlResolved)) {
                                    return new sass.types.String(
                                        pathRelativizer(urlResolved) + queryString);
                                }
                            }
