...
export class WebpackConfigGenerator {
	...
	public generateConfiguration(context: WorkerContext): Promise<WorkerContext> {
		...
		return Promise.resolve(context)
			...
			.then(context => this.configuratorWrapper(Ids.JS_PRE_LOADER, context, new JsPreloadConfigurator()))
			.then(context => this.configuratorWrapper(Ids.TYPESCRIPT_LOADER, context, new TypescriptLoaderConfigurator()))
			.then(context => this.configuratorWrapper(Ids.JS_COMPAT_LOADER, context, new JsCompatConfigurator()))
			...
			.then(context => this.configuratorWrapper(Ids.CSS_EXTRACT_PLUGIN, context, new CssExtractPluginConfigurator()))
			.then(context => this.configuratorWrapper(Ids.PROVIDE_PLUGIN, context, new ProvidePluginConfigurator()))
			.then(context => this.configuratorWrapper(Ids.COPY_PLUGIN, context, new CopyPluginConfigurator()))
			.then(context => this.configuratorWrapper(Ids.CLEAN_OUTPUT_DIR_PLUGIN, context, new CleanOutputDirPluginConfigurator()))
			.then(context => this.configuratorWrapper(Ids.MIN_CHUNK_SIZE_PLUGIN, context, new MinChunkSizePluginConfigurator()))
			.then(context => this.configuratorWrapper(Ids.FILTER_WARNINGS_PLUGIN, context, new FilterWarningsPluginConfigurator()))
	 ...
	 * @param configurator
	 */
	protected configuratorWrapper(identifier: string, context: WorkerContext, configurator: ConfiguratorInterface): Promise<WorkerContext> {
		return context.eventEmitter.emitHook(AssetBuilderEventList.FILTER_CONFIGURATOR, {
			...
			if (args.useConfigurator !== true) return Promise.resolve(context);
	...
	protected mergeAdditionalConfig(context: WorkerContext): Promise<WorkerContext> {
		...
		if (isUndefined(context.app.webpackConfig)) return Promise.resolve(context);
